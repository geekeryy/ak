...
installFile() {
	...
	echo "#!/usr/bin/env sh" >~/.ak/env.sh
	{
		...
		echo "# DO NOT EDIT THIS FILE !!!"
		echo ""
		echo "export AK_ENV=\"true\""
		echo "source $AK_INSTALL_DIR/_ak-script/completion/ak.bash"
	} >>~/.ak/env.sh

	echo "$BINARY_NAME installed into $AK_INSTALL_DIR/$BINARY_NAME"

	# 根据用户shell类型 确保~/.bashrc存在 source ~/.ak/env.sh
	if [ "$SHELL" = "/bin/bash" ]; then
		if ! grep -q "source ~/.ak/env.sh" ~/.bashrc; then
			echo "source ~/.ak/env.sh" >>~/.bashrc
		fi
	elif [ "$SHELL" = "/bin/zsh" ]; then
		if ! grep -q "source ~/.ak/env.sh" ~/.zshrc; then
			echo "source ~/.ak/env.sh" >>~/.zshrc
		fi
	else
		echo "Execute the following command to add the completion function:"
		echo ""
		echo "      echo \"source ~/.ak/env.sh\" >> ~/.bashrc"
	fi
	echo ""
